...
(
  ...
  second_planned_departure_time datetime,
  first_planned_arrival_time datetime
...
(
  ...
  second_planned_departure_time,first_planned_arrival_time
)
select 
	ft.year as year,
    ...
    null as second_planned_departure_time,
    ft.planned_arrival_time as first_planned_arrival_time
...
-- 	ft2.planned_arrival_local_hour as planned_arrival_local_hour,
	null as planned_arrival_time,
	null as planned_arrival_tz,
...
-- 	TIMESTAMPDIFF(minute, ft2.planned_departure_time, ft1.planned_arrival_time) as layover_duration,
	...
	null as second_planned_departure_time,
    ft1.planned_arrival_time as first_planned_arrival_time
from temp_itineraries ti
join flights ft1 on ft1.id = ti.first_flight_id
join flights ft2 on ft2.id = ti.second_flight_id
where ti.num_flights = 2;

update itineraries set second_operating_carrier = (select carrier from flights where flights.id = itineraries.second_flight_id)
	where exists (select carrier from flights where flights.id = itineraries.second_flight_id);
update itineraries set destination = (select destination from flights where flights.id = itineraries.second_flight_id)
	where exists (select destination from flights where flights.id = itineraries.second_flight_id);
update itineraries set planned_arrival_time = (select planned_arrival_time from flights where flights.id = itineraries.second_flight_id)
	where exists (select planned_arrival_time from flights where flights.id = itineraries.second_flight_id);
update itineraries set planned_arrival_tz = (select planned_arrival_tz from flights where flights.id = itineraries.second_flight_id)
	where exists (select planned_arrival_tz from flights where flights.id = itineraries.second_flight_id);
update itineraries set planned_arrival_local_hour = (select planned_arrival_local_hour from flights where flights.id = itineraries.second_flight_id)
	where exists (select planned_arrival_local_hour from flights where flights.id = itineraries.second_flight_id);
update itineraries set multi_carrier_flag = 1 where num_flights = 2 and first_operating_carrier <> second_operating_carrier;
-- alter table itineraries add second_planned_departure_time datetime;
-- alter table itineraries add first_planned_arrival_time datetime;
update itineraries set second_planned_departure_time = (select planned_departure_time from flights where flights.id = itineraries.second_flight_id)
	where exists (select planned_departure_time from flights where flights.id = itineraries.second_flight_id);
-- update itineraries set first_planned_arrival_time = (select planned_arrival_time from flights where flights.id = itineraries.first_flight_id)
-- 	where exists (select planned_arrival_time from flights where flights.id = itineraries.first_flight_id);  
update itineraries set layover_duration = TIMESTAMPDIFF(minute, second_planned_departure_time, first_planned_arrival_time) where num_flights = 2;
alter table itineraries drop second_planned_departure_time;
alter table itineraries drop first_planned_arrival_time;
-- 161848110
